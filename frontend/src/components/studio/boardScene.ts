 ...
 *
 * THE ONLY MODULE IN THIS APPLICATION THAT IMPORTS `three`. It stays behind
 * the lazy import of BoardSurface, so the button that opens the board stays out
 * of the same module graph and opening the map screen does not fetch half a
 * megabyte to draw a 34 px button. Its context is never released on a
 * workspace switch (see the host div in BoardSurface).
 *
