 ...
 *
 * WHICH DEM, and why that has to be said out loud. MapLibre lifts terrain from
 * a `raster-dem` SOURCE, which is tiles on the Web Mercator grid, so the relief
 * drawn here is a global mosaic and nothing an analysis produced: no product in
 * this application reads elevation, and the rasters laid over the relief are
 * renderings of analyses over imagery. A reader must not take the shading for
 * part of a result. That is what `ELEVATION_CREDIT` is for: it names the source
 * in the title bar whenever relief is on, beside the imagery credit.
 *
 ...
 * be named, and naming it is also what keeps its shading from being read as
 * part of an analysis.
 */
...
export function setTerrainEnabled(map: MapLibreMap, on: boolean): void {
  ...
  /*
    ...
    on a screen where results are read against the ground they cover is not a
    matter of taste.
  */
