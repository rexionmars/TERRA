...
/*
  THE APCA BLOCK BELOW WAS ALMOST NOT WRITTEN HERE.

  It arrived by overwriting this file wholesale, which destroyed the twelve
  cases above -- the ones that keep luminance() from being checked against its
  own output. They are back from git, and the two subjects now share a file
  because they share a module: one guards the ratio the palette is gated on,
  the other the figure reported beside it.
*/
