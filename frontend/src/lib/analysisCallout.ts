 ...
 *
 * THE PRODUCT THIS FILE WAS WRITTEN FOR, AND THE ONE IT DID NOT COVER. The
 * argument at the top of this module is a counting one: every product arrives
 * with a reading, the board has a fixed number of regions, and a reading that
 * can be an entry in the tree is one the board does not have to make room for.
 * Two entries were written -- both grid products -- and wind, which produces
 * figures and no raster at all, had neither. It appeared in the Data tab,
 * which lists rasters, in no way; and in the Analyses tab, which lists
 * readings, in no way either. A finished run reported nothing anywhere and the
 * empty-state text went on saying no reading had been made.
 *
