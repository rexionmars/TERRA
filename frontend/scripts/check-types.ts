 ...
 *
 * What is compared: for every Go struct in backend/types.go that has a
 ...
 *   - Go types declared outside backend/types.go, and TypeScript types that are
...
const GO = join(here, "..", "..", "backend", "types.go")
...
/**
 * The structs declared in backend/types.go.
 *
...
function goStructs(): GoStruct[] {
  const lines = readFileSync(GO, "utf8").split("\n")
  const out: GoStruct[] = []
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!tag) {
      throw new Error(
        `${GO}:${at}: cannot read a JSON name from "${line}" in struct ` +
    ...
    if (named === "") {
      ...
      if (!field) {
        throw new Error(`${GO}:${at}: empty json name and no field name in "${line}"`)
      }
  ...
  if (current) throw new Error(`${GO}: struct ${current.name} is never closed`)
  return out
}
...
for (const struct of structs) {
  ...
  for (const field of struct.fields) {
    ...
    console.error(
      `DRIFT  ${struct.name}.${field}: in backend/types.go (line ${struct.line}), ` +
  ...
  for (const field of iface) {
    ...
    console.error(
      `DRIFT  ${struct.name}.${field}: in types.ts, missing from ` +
        `backend/types.go (struct at line ${struct.line})`
    )
    failed++
  }
}
...
for (const [name, exempt] of Object.entries(EXEMPT)) {
  ...
  for (const [field, reason] of Object.entries(exempt.goOnly ?? {})) {
    if (!struct.fields.includes(field)) {
      console.error(
        `STALE  exemption ${name}.${field} ("${reason}"): gone from backend/types.go`
      )
      failed++
  ...
  for (const [field, reason] of Object.entries(exempt.tsOnly ?? {})) {
    if (!iface.includes(field)) {
      ...
      console.error(
        `STALE  exemption ${name}.${field} ("${reason}"): backend/types.go carries it now`
      )
      failed++
    }
  }
...
console.log(
  `\n${structs.length} struct(s) in backend/types.go, ` +
