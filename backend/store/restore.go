...
func (s *Store) InspectBackup(archivePath string) (*RestorePreview, error) {
	...
	if err != nil {
		preview.Problem = "this file is not a readable ZIP archive"
		return preview, nil
	}

	manifest, hasDB, err := readManifest(zr)
	if err != nil {
...
func (s *Store) RestoreBackup(archivePath string) (*RestoreResult, error) {
	...
	zr, err := zip.NewReader(bytes.NewReader(raw), int64(len(raw)))
	if err != nil {
		return nil, fmt.Errorf("this file is not a readable ZIP archive")
	}
	manifest, hasDB, err := readManifest(zr)
	if err != nil {
		return nil, err
	...
	if err := os.Rename(staging, dataDir); err != nil {
		...
		return nil, fmt.Errorf("putting the restored data in place: %w", err)
	}
	return result, nil
}
