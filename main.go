...
that shipped a cache needing a package nothing declared.
	...
	err := wails.Run(&options.App{
		...
		*/
		...
		AssetServer: &assetserver.Options{
			Assets: assets,
			/*
				Run images are fetched over HTTP, not returned through a bound
				method: everything a bound method returns is marshalled to JSON
				and handed to the webview as a string, so a raster would have to
				cross as base64. See runOverlayMiddleware.
				...
				path with index.html rather than a miss. Middleware runs ahead of
				Assets, so the route is decided here and everything else falls
				through untouched.
			*/
			Middleware: app.runOverlayMiddleware,
		},
