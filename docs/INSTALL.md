...
Prithvi, for instance — choose it in **Settings → System**. `TERRA_PYTHON`
...
1. `TERRA_PYTHON` — absolute path to the interpreter
...
Settings → System is the way to set this. `TERRA_PYTHON` reaches only a
...
```bash
export TERRA_PYTHON="$HOME/venvs/terra/bin/python"
...
| Variable | Purpose |
|----------|---------|
| `TERRA_PYTHON` | Python interpreter for the sidecar. Overrides Settings → System |
| `TERRA_APP_DIR` | Directory containing `sidecar/`, `areas/`, `model/` |
| `TERRA_MODEL_DIR` | Override trained model directory (default `model/`) |
| `TERRA_ROOT` | Parent repo root used for legacy MapBiomas paths |
...
> **Renamed from `GEOSENSE_*`.** These were `GEOSENSE_PYTHON`,
> `GEOSENSE_APP_DIR`, `GEOSENSE_MODEL_DIR` and `GEOSENSE_ROOT`; geosense is the
> research repository TERRA grew out of, not the application. The old names are
> no longer read at all. If any are still set in a shell profile or CI job they
> now have no effect, so Settings → System lists them where it reports the
> resolved paths — an environment variable that silently stopped working is
> otherwise very hard to notice.
>
> The data directory moved with them, from `geosense-infer` to `terra` under
> the user config directory. TERRA moves an existing one on first start; if
> both exist the newer location is used and nothing is touched.
