...
func TestMinPythonMatchesDoctor(t *testing.T) {
	...
	// Anchored on go.mod rather than on a count of parent directories, which
	// is what broke when this package moved one level deeper: two Dir calls
	// resolved to internal/ and looked for internal/sidecar/doctor.py.
	...
	doctor := filepath.Join(dir, "sidecar", "doctor.py")
