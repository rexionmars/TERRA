...
func TestResearchPackEnergyAndWindTables(t *testing.T) {
	var energy analysis.EnergyModelAnalysis
	...
	result := &analysis.PredictResult{EnergyModel: &energy, Wind: &wind}
	meta := analysis.ResearchExportMeta{ModelKind: "NASA POWER", AoiLabel: "Propriedade B"}
...
func TestResearchPackTerrainOnly(t *testing.T) {
	shadingMean := 1.234
	shadingMax := 41.5
	terrain := &analysis.SolarTerrainAnalysis{
	...
	zipBytes, err := BuildResearchPackZIP(
		analysis.ResearchExportMeta{AoiLabel: "Propriedade B"},
		&analysis.PredictResult{SolarTerrain: terrain},
		t.TempDir(),
	)
	if err != nil {
		t.Fatal(err)
