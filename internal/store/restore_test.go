...
/*
TestInspectBackupSeparatesAnUnreadableFileFromANonArchive covers the split that
opening the archive lazily made necessary.

zip.OpenReader reports "could not open this file" and "this is not a ZIP" as
one error, and the two are different answers: a file the user cannot read is a
failure, a file that is not a backup is a preview saying so. A path that does
not exist must not come back as a preview claiming the file is not a ZIP.
*/
func TestInspectBackupSeparatesAnUnreadableFileFromANonArchive(t *testing.T) {
	s := openTestStore(t)

	missing := filepath.Join(t.TempDir(), "not-here.zip")
	preview, err := s.InspectBackup(missing)
	if err == nil {
		t.Fatalf("a missing archive was described rather than refused: %+v", preview)
	}
	if strings.Contains(err.Error(), "not a readable ZIP") {
		t.Errorf("a missing file was reported as a malformed one: %v", err)
	}
