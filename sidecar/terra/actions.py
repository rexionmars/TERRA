"""
The actions not yet moved into a slice, and the Sentinel-2 reader they share.

This is where the sidecar was before the package existed: seventeen action
functions on top of one reader for Sentinel-2 L2A, covering land cover, water,
energy, terrain and canopy at once. It is being emptied a slice at a time, into
`terra.landcover`, `terra.water`, `terra.energy`, `terra.canopy` and the
readers under `terra.imagery` and `terra.terrain`, readers first because one of
them collapses three separate Planetary Computer clients into one. Nothing new
is added here.
...
An action reads its request, runs the product, and writes a single JSON object
to stdout. The shapes of those requests and responses are the contract with
`internal/analysis/runner.go`, which builds each request and parses each
response; the classification pair is:
